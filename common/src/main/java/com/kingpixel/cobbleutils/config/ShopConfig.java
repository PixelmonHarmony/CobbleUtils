...
public class ShopConfig {
  ...
  public static void createConfigIfNotExists(String mod_id, String pathShops, String path) {
    CompletableFuture<Boolean> futureRead = Utils.readFileAsync(path, "shopconfig.json",
      el -> {
        Gson gson = Utils.newGson();
        ShopConfig config = gson.fromJson(el, ShopConfig.class);
        ShopConfigMenu existingShopMenu = config.shop;
    ...
    if (!futureRead.join()) {
      CobbleUtils.LOGGER.info("No shopconfig.json file found. Creating a new one.");
      ShopConfig newConfig = new ShopConfig();
      Gson gson = Utils.newGson();
      String data = gson.toJson(newConfig);
      CompletableFuture<Boolean> futureWrite = Utils.writeFileAsync(path, "shopconfig.json", data);
      if (!futureWrite.join()) {
        CobbleUtils.LOGGER.fatal("Could not write shopconfig.json file.");
      } else {
        CobbleUtils.LOGGER.info("shopconfig.json created successfully.");
      }
    }
  }
  ...
  public void init(String pathShop, String mod_id, String pathShops) {
    ...
    if (CobbleUtils.config.isDebug()) {
      CobbleUtils.LOGGER.info("Shops loaded: " + shops);
      for (int i = 0; i < 5; i++) {
        CobbleUtils.LOGGER.info("");
      }
      shops.keySet().forEach(shopMod -> {
        CobbleUtils.LOGGER.info("ShopMod: " + shopMod);
        shops.get(shopMod).forEach(shop -> {
          CobbleUtils.LOGGER.info("Shop: " + shop);
        });
      });
    }
